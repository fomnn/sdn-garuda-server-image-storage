...
export default eventHandler(async (event) => {
  ...
  if (oldImageExt !== newImageExt) {
    ...
    const imagePath = `/images/students/${newImageName}`
    const imageUrl = `${baseUrl}/api${imagePath}`

    // remove old image
    await useStorage('fs').removeItem(`/images/students/${name}`)
  ...
  // menyimpan buffer image ke storage
  await useStorage('fs').setItemRaw(`/images/students/${name}`, Buffer.from(arrayBuffer))

  const imagePath = `/images/students/${name}`
  const imageUrl = `${baseUrl}/api${imagePath}`
