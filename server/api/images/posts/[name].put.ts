...
export default eventHandler(async (event) => {
  ...
  if (oldImageExt !== newImageExt) {
    ...
    const imagePath = `/images/posts/${newImageName}`
    const imageUrl = `${baseUrl}/api${imagePath}`

    // remove old image
    await useStorage('fs').removeItem(`/images/posts/${name}`)
  ...
  // menyimpan buffer image ke storage
  await useStorage('fs').setItemRaw(`/images/posts/${name}`, Buffer.from(arrayBuffer))

  const imagePath = `/images/posts/${name}`
  const imageUrl = `${baseUrl}/api${imagePath}`
